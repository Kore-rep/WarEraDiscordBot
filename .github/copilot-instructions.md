...
- discord.js v14 bot wrapping `warera-sdk` (a public git dependency fetched from GitHub; no local checkout needed). It runs in **many Discord servers** — always scope reads, writes, and messages by `serverId` (guild id).
...
- Verify with `npm run build` and `npm test`. `npm install` fetches the `warera-sdk` git dependency (needs `git` + GitHub access).
